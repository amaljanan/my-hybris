...
public class NewSyncExclusiveTest extends HybrisJUnit4Test
{
	...
	public void testExclusiveSync()
	{
		...
		exclJob2.setCreateNewItems(true); // create new items in full sync
	...
	@Test
	public void testCopyByValueAttributeExclusiveSync()
	{
		assertNull(catalogManager.getCounterpartItem(product1, tgt));
		assertNull(catalogManager.getCounterpartItem(product2, tgt));
		assertNull(catalogManager.getCounterpartItem(product3, tgt));
		...
		final CatalogVersionSyncJob exclJob = createSyncJob("excl", src, tgt, threads, true);
		final SyncAttributeDescriptorConfig unitCfg = exclJob.getConfigFor(unitAd, true);
		unitCfg.setIncludedInSync(true);
		unitCfg.setCopyByValue(true);

		final CatalogVersionSyncCronJob run = (CatalogVersionSyncCronJob) exclJob.newExecution();
		exclJob.configureFullVersionSync(run);
		exclJob.perform(run, true);
		assertEquals(run.getResult(), run.getSuccessResult());
	}
