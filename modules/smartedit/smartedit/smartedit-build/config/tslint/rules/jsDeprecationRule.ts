...
export class Rule extends Lint.Rules.AbstractRule {
    public static metadata: Lint.IRuleMetadata = {
        ...
        options: {
            ...
            items: {
                type: 'string',
                enum: [OPTION_DEPRECATE_FROM_VERSION]
            }
        },
        type: 'functionality',
    ...
    public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
        return this.applyWithWalker(
            new DeprecationWalker(sourceFile, this.ruleName, {
                deprecateFromMajorVersion: this.ruleArguments[0],
                deprecateFromMinorVersion: this.ruleArguments[1]
            })
        );
    }
}
...
export class DeprecationWalker extends Lint.AbstractWalker<Options> {
    public walk(sourceFile: ts.SourceFile) {
        ...
        utils.forEachTokenWithTrivia(
            sourceFile,
            (fullText: string, kind: ts.SyntaxKind, range: ts.TextRange, parent: ts.Node) => {
                if (kind === ts.SyntaxKind.MultiLineCommentTrivia) {
                    const commentText: string = fullText.slice(range.pos, range.end);
                    const deprecatedVersion: HybrisVersion = this.getDeprecatedVersionFromAnnotation(
                        commentText
                    );
                    if (
                        deprecatedVersion &&
                        this.isVersionDeprecatedFrom(
                            this.options.deprecateFromMajorVersion,
                            this.options.deprecateFromMinorVersion,
                            deprecatedVersion
                        )
                    ) {
                        const startCodeSnippet: number = parent.getStart(parent.getSourceFile());
                        const endCodeSnippet: number = fullText.indexOf('\n', startCodeSnippet);
                        const codeSnippet: string = fullText.slice(
                            startCodeSnippet,
                            endCodeSnippet
                        );
                        // tslint:disable-next-line
                        console.info(
                            'INFO: ' +
                                ...
                                ' : deprecated code since version ' +
                                deprecatedVersion.toString +
                                ' : ' +
                                codeSnippet
                        );
    ...
    public isVersionDeprecatedFrom(
        fromMajor: number,
        fromMinor: number,
