            ...
            definition.imports.forEach((importStatement, index) => {
                ...
                if (typeof importStatement === 'string') {
                    ...
                    if (importStatement.providers) {
                        addArrayOfProvidersToModule(
                            moduleWithProviders,
                            (importStatement as any).providers
                        );
                    }
                } else {
                    throw new Error(
...
function addFullComponentGraphToModule(
    ...
    if (!definition) {
        ...
        throw new Error(
            `${componentConstructorName} component was imported into ${
                module.name
            } module but doesn't seem to have been @SeComponent or @SeDirective annotated`
        );
    }
