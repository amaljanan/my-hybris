                    ...
                    if (fn === 'component') {
                        ...
                        try {
                            if (
                                ...
                                $provide.decorator(
                                    recipeName,
                                    ($delegate: (...param: any[]) => any) => {
                                        const iterableDelegate = this._getIterableDelegate(
                                            $delegate
                                        );
                                        ...
                                        originals.forEach((original) => {
                                            if (!iterableDelegate[original.name].__DECORATED__) {
                                                const self = this;
                                                const isEmpty = self.functionsUtils.isEmpty(
                                                    original.fn
                                                );
                                                if (isEmpty) {
                                                    iterableDelegate[original.name] = function() {
                                                        'proxyFunction';
                                                        const result = original.fn.apply(
                                                            this,
                                                            arguments
                                                        );
