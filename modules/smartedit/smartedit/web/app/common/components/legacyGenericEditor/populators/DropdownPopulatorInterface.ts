        ...
        return this.languageService.getResolveLocale().then((isocode: string) => {
            return this.lodash.filter(items, (item: GenericEditorOption) => {
                ...
                if (this.lodash.isObject(item.label)) {
                    isocode = (item.label as TypedMap<string>)[isocode]
                        ? isocode
                        : Object.keys(item.label as TypedMap<string>)[0];
                    labelValue = (item.label as TypedMap<string>)[isocode];
                } else {
                    labelValue = this.translateService
                        ? this.translateService.instant(item.label as string)
                        : (item.label as string);
                }
                return (
                    labelValue &&
                    (labelValue as string).toUpperCase().indexOf(searchTerm.toUpperCase()) > -1
                );
            });
        });
    }
